...
export async function exchangeZaloCode(code: string): Promise<{ access_token: string; uid: string; name: string; picture: { data: { url: string } } }> {
  ...
  if (!res.ok) throw new Error('Zalo token exchange failed')
